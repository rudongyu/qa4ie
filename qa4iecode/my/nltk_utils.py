...
def span_prec(true_span, pred_span):
    #logging.warning(true_span)
    #logging.warning(pred_span)
    overlap = my_span_overlap(true_span, pred_span)
    if overlap is None:
        return 0
    return len(overlap) / len(pred_span)
...
def span_recall(true_span, pred_span):
    ...
    if overlap is None:
        return 0
    return len(overlap) / len(true_span)

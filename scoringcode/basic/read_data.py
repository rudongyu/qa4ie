...
class DataSet(object):
    ...
    def _sort_key(self, idx):
        rx = self.data['*sents'][idx]
        x = self.shared['sents'][rx[0]][rx[1]][rx[2]]
        return max(map(len, x))
        # sents = self.data['sents'][idx][0]
        # return len(sents)
    ...
    def get_batches(self, batch_size, num_batches=None, shuffle=False, cluster=False):
        ...
        :return:
        """
        num_batches_per_epoch = int(math.floor(self.num_examples / batch_size))
        if num_batches is None:
            num_batches = num_batches_per_epoch
        num_epochs = int(math.ceil(num_batches / num_batches_per_epoch))
        valid_idxs = self.valid_idxs[:num_batches_per_epoch*batch_size]

        if shuffle:
            random_idxs = random.sample(valid_idxs, len(valid_idxs))
            if cluster:
            ...
            raw_grouped = lambda: list(grouper(valid_idxs, batch_size))
            grouped = raw_grouped
        ...
        for _ in range(num_batches):
            ...
            # batch_data.update(self.get_by_idxss(batch_idxs))
...
def read_data(config, data_type, ref, data_filter=None):
    # data_path = os.path.join(config.data_dir, config.data_scale, "data_{}.json".format(data_type))
    # shared_path = os.path.join(config.data_dir, config.data_scale, "shared_{}.json".format(data_type))
...
def update_config(config, data_sets):
    ...
    for data_set in data_sets:
        ...
        for idx in data_set.valid_idxs:
            ...
            q = data['q'][idx]
            try:
                sents = shared['sents'][rx[0]][rx[1]][rx[2]]
            except Exception as e:
                print(rx, shared['sents'])
                exit()
            #sents = data['sents'][idx]
            config.max_para_size = 1#max(config.max_para_size, sum(map(len, sents)))
            config.max_num_sents = 1#max(config.max_num_sents, len(sents))
    ...
    print(config.max_num_sents, config.max_sent_size, config.max_para_size, config.max_word_size)

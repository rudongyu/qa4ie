...
from basic.evaluator import Evaluation, ScoreEvaluation#, F1Evaluation
...
class GraphHandler(object):
    ...
    def dump_score(self, e, part = 0, path=None):
        assert isinstance(e, ScoreEvaluation)
        path = path or os.path.join(self.config.score_dir, "{}-{}-{}.json".format(e.data_type, str(e.global_step).zfill(6), part))
        if(part):
            path = path or os.path.join(self.config.score_dir, "{}-{}-{}.json".format(e.data_type, str(e.global_step).zfill(6), part))
        with open(path, 'w') as fh:
            json.dump(e.dict, fh)

...
class Evaluation(object):
    def __init__(self, data_type, global_step, idxs, score, tensor_dict=None):
        self.data_type = data_type
        self.global_step = global_step
        self.idxs = idxs
        #self.yp = yp
        self.score = score
        self.num_examples = len(score)
    ...
    def __add__(self, other):
        ...
        return Evaluation(self.data_type, self.global_step, new_idxs, new_score, tensor_dict=new_tensor_dict)

    def __radd__(self, other):
        return self.__add__(other)

class ScoreEvaluation(object):
    def __init__(self, data_type, global_step, idxs, scorep, rsent, label, qaid, sentslen):
        assert len(scorep) == len(scorep), (len(scorep), len(rsent), len(label))
        self.data_type= data_type
        self.global_step = global_step
        self.idxs = idxs
        self.num_examples = len(scorep)
        self.scorep = list(scorep)
        self.rsent = rsent
        self.label = list(label)
        self.qaid = list(qaid)
        self.sentslen = list(sentslen)
        # print(qaid)
        self.dict = {
    ...
    def __add__(self, other):
        ...
        assert self.global_step == other.global_step
        new_idxs = self.idxs + other.idxs
        new_rsent = self.rsent + other.rsent
        new_scorep = self.scorep + other.scorep
        new_label = self.label + other.label
        new_qaid = self.qaid + other.qaid
        new_sentslen = self.sentslen + other.sentslen
        return ScoreEvaluation(self.data_type, self.global_step, new_idxs, new_scorep, new_rsent, new_label, new_qaid, new_sentslen)
...
class Evaluator(object):
    ...
    def get_evaluation(self, sess, batch):
        ...
        global_step, score, vals = sess.run([self.global_step, self.score, list(self.tensor_dict.values())], feed_dict=feed_dict)
        #yp = yp[:data_set.num_examples]
        score = score[:data_set.num_examples]
        tensor_dict = dict(zip(self.tensor_dict.keys(), vals))
        e = Evaluation(data_set.data_type, int(global_step), idxs, score.tolist(), tensor_dict=tensor_dict)
        return e
...
class ScoreEvaluator(Evaluator):
    def __init__(self, config, model, tensor_dict=None):
        super(ScoreEvaluator, self).__init__(config, model, tensor_dict=tensor_dict)
        self.pred_score = model.probs

    def get_evaluation(self, sess, batch):
        idxs, data_set = batch
        feed_dict = self.model.get_feed_dict(data_set, False, supervised=False)
        global_step, scorep = sess.run([self.global_step, self.pred_score], feed_dict=feed_dict)
        scorep = scorep[:data_set.num_examples, 1]
        scorep = np.reshape(scorep[:data_set.num_examples], [data_set.num_examples])
        rsent = data_set.data['*sents']
        label = data_set.data['label']
        qaid = data_set.data['id']
        sentslen = data_set.data['sentslen']
        # print(qaid)
        # print( type(scorep), type(score), type(label) )
        e = ScoreEvaluation(data_set.data_type, int(global_step), idxs, scorep.tolist(), rsent, label, qaid, sentslen)
        return e

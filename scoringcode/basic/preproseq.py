...
def get_args():
    ...
    target_dir = "dataout/qa4ie/{}/seq".format(file_size)
...
def prepro_each(args, data_type, start_ratio=0.0, stop_ratio=1.0, out_name="default", in_path=None):
    ...
    q, cq, y, rx, rcx, ids = [], [], [], [], [], []
    x, cx = [], []
    filter_dict = json.load(open("sentid-infer400-700.json", 'r'))
    answerss = []
    p = []
    total = 0
    ...
    print("{}/{}".format(len(q),total))

    # add context here
    data = {'q': q, 'cq': cq, 'y': y, '*x': rx, '*cx': rcx,
            'ids': ids, 'answerss': answerss, '*p': rx}

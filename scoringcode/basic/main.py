...
from basic.evaluator import Evaluator, ScoreEvaluator#ForwardEvaluator, MultiGPUF1Evaluator, Evaluator
...
def set_dirs(config):
    ...
    config.score_dir = os.path.join(config.out_dir, "score")
...
def _train(config):
    ...
    evaluator = Evaluator(config, model, tensor_dict=model.tensor_dict if config.vis else None)
    graph_handler = GraphHandler(config, model)  # controls all tensors and variables in the graph, including loading /saving

    # Variables
    configgpu = tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)
...
def _test(config):
    ...
    models = get_multi_gpu_models(config)
    model = models[0]
    evaluator = ScoreEvaluator(config, model, tensor_dict=model.tensor_dict if config.vis else None)
...
#     models = get_multi_gpu_models(config)
#     model = models[0]
#     evaluator = Evaluator(config, model, tensor_dict=models[0].tensor_dict if config.vis else None)
#     graph_handler = GraphHandler(config, model)
...
def _inference(config):
    inference_handler = InferenceHandler(config, "data/inference/", True)

    if config.use_glove_for_unk:
        word2vec_dict = inference_handler.shared['lower_word2vec'] if config.lower_word else test_data.shared['word2vec']
        new_word2idx_dict = inference_handler.shared['new_word2idx']
        idx2vec_dict = {idx: word2vec_dict[word] for word, idx in new_word2idx_dict.items()}
        new_emb_mat = np.array([idx2vec_dict[idx] for idx in range(len(idx2vec_dict))], dtype='float32')
        config.new_emb_mat = new_emb_mat
    ...
    # with open('inf.json', 'r') as fh:
    #     data = json.load(fh)
    #     for i, x in enumerate(data['sents']):
    #         label = data['label'][i]
    #         qa= data['qa'][i]
    for batch_idx in tqdm(range(inference_handler.batch_num)):
        batch_data = inference_handler.get_batch()

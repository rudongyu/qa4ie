...
class Model(object):
    def __init__(self, config, scope, rep=True):
        ...
        self.score_ = tf.placeholder('int32', [N, None], name="score_")
        ...
        self.summary = tf.summary.merge(tf.get_collection("summaries"c, scope=self.scope))
    ...
    def _build_loss(self):
        #self.loss = tf.reduce_sum(tf.square(self.score_ -  self.score))
        labels = tf.one_hot(self.score_, 2)
        self.loss = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(labels = labels, logits = self.score))
    ...
    def get_feed_dict(self, batch, is_train, supervised=True):
        ...
        X = batch.data['sents']
        CX = batch.data['csents']

        if supervised:
            score_ = np.reshape(batch.data['label'], (N, 1))
            feed_dict[self.score_] = score_

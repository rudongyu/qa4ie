...
class Evaluator(object):
    ...
    def get_evaluation(self, sess, batch):
        ...
        id2answer_dict = {}
        id2score_dict = {}
        for id_, yp_, yp_m, x_ in zip(data_set.data['id'], yp_list, yp_mat, data_set.data['x']):#, data_set.data['p']):
        ...
        assert len(data_set.data['id']) == len(yp_list)
        ...
        for i, yi in enumerate(y):
            for ij, yij in enumerate(yi):
                if self.config.data_type == 'span':
                    ...
                    ans_sent_index = 0 # default 0 yij[0][0]
                    y[i][ij] = []
                    for loc in yij:
                        y[i][ij].append([ans_sent_index, loc])

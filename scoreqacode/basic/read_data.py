...
def read_data(config, data_type, ref, data_filter=None):
    data_path = os.path.join(config.data_dir, config.data_scale, config.data_type, "data_{}.json".format(data_type))
    shared_path = os.path.join(config.data_dir, config.data_scale, config.data_type, "shared_{}.json".format(data_type))
...
def update_config(config, data_sets):
    ...
    for data_set in data_sets:
        ...
        for idx in data_set.valid_idxs:
            rx = data['*x'][idx]
            q = data['q'][idx]
            sents = [[word for i in rx[2] for word in shared['x'][rx[0]][rx[1]][i]]]
